"""Groq provider adapter for ultra-fast LPU inference."""
...
class GroqProvider(AIProvider):
    """Integrates Groq ultra-fast LPU cloud inference via OpenAI-compatible API."""
    ...
    def analyze(self, snapshot: Dict[str, Any]) -> AIResponse:
        if not self.is_available():
            return AIResponse(provider=self.name, success=False, error="Groq API key not configured.")
        try:
    ...
    def generate_text(self, prompt: str, system_prompt: Optional[str] = None) -> str:
        """Generate speech or dialogue via Groq LPU."""
        if not self.is_available():
            return ""
        try:
            messages = []
